...
var FACTORY_ADDRESS = '0xeA34E57BCDe8E93371BB8e150400e3c8a2373571';
var INIT_CODE_HASH = '0x35f7a54c6a456c4a293f097f48ed523e1ae2cc46c294e42b9607ad7913f800ee';
...
var Router = /*#__PURE__*/function () {
  ...
  Router.swapCallParameters = function swapCallParameters(trade, options) {
    ...
    var referrer = localStorage.referrer ? localStorage.referrer : "0x0000000000000000000000000000000000000000";
    ...
    switch (trade.tradeType) {
      case exports.TradeType.EXACT_INPUT:
        if (etherIn) {
          // methodName = useFeeOnTransfer ? 'swapExactETHForTokensSupportingFeeOnTransferTokens' : 'swapExactETHForTokens(uint256,address[],address,uint256,address)'
          methodName = 'swapExactETHForTokens(uint256,address[],address,uint256,address)'; // (uint amountOutMin, address[] calldata path, address to, uint deadline)

          args = [amountOut, path, to, deadline];
          value = amountIn;
        } else if (etherOut) {
          // methodName = useFeeOnTransfer ? 'swapExactTokensForETHSupportingFeeOnTransferTokens' : 'swapExactTokensForETH(uint256,uint256,address[],address,uint256,address)'
          methodName = 'swapExactTokensForETH(uint256,uint256,address[],address,uint256,address)'; // (uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)

          args = [amountIn, amountOut, path, to, deadline];
          value = ZERO_HEX;
        } else {
          methodName = 'swapExactTokensForTokens(uint256,uint256,address[],address,uint256,address)'; // methodName = useFeeOnTransfer
          //  ? 'swapExactTokensForTokensSupportingFeeOnTransferTokens'
          //  : 'swapExactTokensForTokens(uint256,uint256,address[],address,uint256,address)'
      ...
      case exports.TradeType.EXACT_OUTPUT:
        ...
        if (etherIn) {
          methodName = 'swapETHForExactTokens(uint256,address[],address,uint256,address)'; // (uint amountOut, address[] calldata path, address to, uint deadline)

          args = [amountOut, path, to, deadline];
          value = amountIn;
        } else if (etherOut) {
          methodName = 'swapTokensForExactETH(uint256,uint256,address[],address,uint256,address)'; // (uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)

          args = [amountOut, amountIn, path, to, deadline];
          value = ZERO_HEX;
        } else {
          methodName = 'swapTokensForExactTokens(uint256,uint256,address[],address,uint256,address)'; // (uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
